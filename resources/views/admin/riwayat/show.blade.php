	...
	<x-card title="Berikut hasil diagnosa penyakit">
		...
		<table class="table table-hover border">
			<thead class="thead-light">
				<tr>
					<th>Gejala yang di alami tanaman saat ini</th>
		...
		<div class="mt-5">
			<div class="alert alert-success">
					<h1 class="font-weight-bold">Kesimpulan</h1>
					<p class="h3">Berdasarkan dari gejala yang kamu pilih atau alami juga berdasarkan Role/Basis aturan yang sudah ditentukan oleh seorang pakar penyakit maka perhitungan Algoritma Certainty Factor mengambil nilai CF yang paling tinggi yakni <b class="text-danger">{{ number_format(unserialize($riwayat->cf_max)[0], 3) }} ({{ number_format(unserialize($riwayat->cf_max)[0], 3) * 100 }}%)</b> yaitu <b>{{ unserialize($riwayat->cf_max)[1] }}.</b>
					<b>
						@if (isset($diagnosa['solusi']))
							<h1 class="font-weight-bold">Solusi</h1>
							<p class="h3">{{ $diagnosa['solusi'] }}</p>
						@endif
					</b>
					</p>
			</div>
